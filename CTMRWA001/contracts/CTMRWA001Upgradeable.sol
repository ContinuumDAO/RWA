...
import "./ICTMRWA001Upgradeable.sol";
import "./IERC721ReceiverUpgradeable.sol";
import "./ICTMRWA001ReceiverUpgradeable.sol";
import "./extensions/IERC721EnumerableUpgradeable.sol";
import "./extensions/IERC721MetadataUpgradeable.sol";
import "./extensions/ICTMRWA001MetadataUpgradeable.sol";
import "./periphery/interface/ICTMRWA001MetadataDescriptorUpgradeable.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract CTMRWA001Upgradeable is Initializable, ContextUpgradeable, ICTMRWA001MetadataUpgradeable, IERC721EnumerableUpgradeable {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            ...
            interfaceId == type(ICTMRWA001Upgradeable).interfaceId ||
            interfaceId == type(IERC721Upgradeable).interfaceId ||
            interfaceId == type(ICTMRWA001MetadataUpgradeable).interfaceId ||
    ...
    function transferFrom(
        ...
        _mint(to_, newTokenId, CTMRWA001Upgradeable.slotOf(fromTokenId_), 0);
    ...
    function approve(address to_, uint256 tokenId_) public payable virtual override {
        ...
        require(
            _msgSender() == owner || CTMRWA001Upgradeable.isApprovedForAll(owner, _msgSender()),
    ...
    function tokenByIndex(uint256 index_) public view virtual override returns (uint256) {
        require(index_ < CTMRWA001Upgradeable.totalSupply(), "CTMRWA001: global index out of bounds");
    ...
    function tokenOfOwnerByIndex(address owner_, uint256 index_) public view virtual override returns (uint256) {
        require(index_ < CTMRWA001Upgradeable.balanceOf(owner_), "CTMRWA001: owner index out of bounds");
    ...
    function _isApprovedOrOwner(address operator_, uint256 tokenId_) internal view virtual returns (bool) {
        ...
        return (
            ...
            CTMRWA001Upgradeable.getApproved(tokenId_) == operator_
        );
    }

    function _spendAllowance(address operator_, uint256 tokenId_, uint256 value_) internal virtual {
        uint256 currentAllowance = CTMRWA001Upgradeable.allowance(tokenId_, operator_);
    ...
    function _approve(address to_, uint256 tokenId_) internal virtual {
        ...
        emit Approval(CTMRWA001Upgradeable.ownerOf(tokenId_), to_, tokenId_);
    }
    ...
    function _transferTokenId(
        ...
        require(CTMRWA001Upgradeable.ownerOf(tokenId_) == from_, "CTMRWA001: transfer from invalid owner");
        require(to_ != address(0), "CTMRWA001: transfer to the zero address");

        uint256 slot = CTMRWA001Upgradeable.slotOf(tokenId_);
        uint256 value = CTMRWA001Upgradeable.balanceOf(tokenId_);
    ...
    function _checkOnCTMRWA001Received( 
        ...
        address to = CTMRWA001Upgradeable.ownerOf(toTokenId_);
        if (_isContract(to)) {
            try IERC165(to).supportsInterface(type(ICTMRWA001ReceiverUpgradeable).interfaceId) returns (bool retval) {
                if (retval) {
                    bytes4 receivedVal = ICTMRWA001ReceiverUpgradeable(to).onCTMRWA001Received(_msgSender(), fromTokenId_, toTokenId_, value_, data_);
                    return receivedVal == ICTMRWA001ReceiverUpgradeable.onCTMRWA001Received.selector;
                } else {
                    return true;
                }

...
import "./interface/ICTM3525XMetadataDescriptorUpgradeable.sol";
import "../extensions/ICTM3525XMetadataUpgradeable.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract CTM3525XMetadataDescriptorUpgradeable is Initializable, ICTM3525XMetadataDescriptorUpgradeable {

    using Strings for uint256;

    function __CTM3525XMetadataDescriptor_init() internal onlyInitializing {
    }

    function __CTM3525XMetadataDescriptor_init_unchained() internal onlyInitializing {
    }
    function constructContractURI() external view override returns (string memory) {
        ICTM3525XMetadataUpgradeable CTM3525X = ICTM3525XMetadataUpgradeable(msg.sender);
        return 
            string(
                abi.encodePacked(
                    /* solhint-disable */
                    ...
                    Base64.encode(
                        abi.encodePacked(
                            ...
                            _contractImage(),
                            '","valueDecimals":"', 
                            uint256(CTM3525X.valueDecimals()).toString(),
                            '"}'
                        )
                    )
                    /* solhint-enable */
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        abi.encodePacked(
                            ...
                            '","balance":"',
                            CTM3525X.balanceOf(tokenId_).toString(),
                            '","slot":"',
                            CTM3525X.slotOf(tokenId_).toString(),
                            '","properties":',
    ...
    function _tokenName(uint256 tokenId_) internal view virtual returns (string memory) {
        ...
        return 
            string(
                abi.encodePacked(
                    ICTM3525XMetadataUpgradeable(msg.sender).name(), 
                    " #", tokenId_.toString()
                )
            );
    }

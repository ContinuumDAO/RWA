...
import "./interface/ICTM3525XMetadataDescriptor.sol";
import "../extensions/ICTM3525XMetadata.sol";

contract CTM3525XMetadataDescriptor is ICTM3525XMetadataDescriptor {

    using Strings for uint256;

    function constructContractURI() external view override returns (string memory) {
        ICTM3525XMetadata CTM3525X = ICTM3525XMetadata(msg.sender);
        return 
            string(
                abi.encodePacked(
                    /* solhint-disable */
    ...
    function _tokenName(uint256 tokenId_) internal view virtual returns (string memory) {
        ...
        return 
            string(
                abi.encodePacked(
                    ICTM3525XMetadata(msg.sender).name(), 
                    " #", tokenId_.toString()
                )
            );
    }

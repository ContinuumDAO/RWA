...
import "./ICTM3525XUpgradeable.sol";
import "./IERC721ReceiverUpgradeable.sol";
import "./ICTM3525XReceiverUpgradeable.sol";
import "./extensions/IERC721EnumerableUpgradeable.sol";
import "./extensions/IERC721MetadataUpgradeable.sol";
import "./extensions/ICTM3525XMetadataUpgradeable.sol";
import "./periphery/interface/ICTM3525XMetadataDescriptorUpgradeable.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract CTM3525XUpgradeable is Initializable, ContextUpgradeable, ICTM3525XMetadataUpgradeable, IERC721EnumerableUpgradeable {
    ...
    ICTM3525XMetadataDescriptorUpgradeable public metadataDescriptor;

    function __CTM3525X_init(string memory name_, string memory symbol_, uint8 decimals_) internal onlyInitializing {
        __CTM3525X_init_unchained(name_, symbol_, decimals_);
    }

    function __CTM3525X_init_unchained(string memory name_, string memory symbol_, uint8 decimals_) internal onlyInitializing {
        _tokenIdGenerator = 1;
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            ...
            interfaceId == type(ICTM3525XUpgradeable).interfaceId ||
            interfaceId == type(IERC721Upgradeable).interfaceId ||
            interfaceId == type(ICTM3525XMetadataUpgradeable).interfaceId ||
    ...
    function ownerOf(uint256 tokenId_) public view virtual override returns (address owner_) {
        ...
        require(owner_ != address(0), "CTM3525X: invalid token ID");
    }
    ...
    function approve(uint256 tokenId_, address to_, uint256 value_) public payable virtual override {
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId_), "CTM3525X: approve caller is not owner nor approved");
    ...
    function transferFrom(
        ...
        _mint(to_, newTokenId, CTM3525XUpgradeable.slotOf(fromTokenId_), 0);
    ...
    function balanceOf(address owner_) public view virtual override returns (uint256 balance) {
        require(owner_ != address(0), "CTM3525X: balance query for the zero address");
    ...
    function transferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId_), "CTM3525X: transfer caller is not owner nor approved");
    ...
    function approve(address to_, uint256 tokenId_) public payable virtual override {
        ...
        require(
            _msgSender() == owner || CTM3525XUpgradeable.isApprovedForAll(owner, _msgSender()),
            "CTM3525X: approve caller is not owner nor approved for all"
        );
    ...
    function tokenByIndex(uint256 index_) public view virtual override returns (uint256) {
        require(index_ < CTM3525XUpgradeable.totalSupply(), "CTM3525X: global index out of bounds");
    ...
    function tokenOfOwnerByIndex(address owner_, uint256 index_) public view virtual override returns (uint256) {
        require(index_ < CTM3525XUpgradeable.balanceOf(owner_), "CTM3525X: owner index out of bounds");
    ...
    function _setApprovalForAll(
        ...
        require(owner_ != operator_, "CTM3525X: approve to caller");
    ...
    function _isApprovedOrOwner(address operator_, uint256 tokenId_) internal view virtual returns (bool) {
        ...
        return (
            ...
            CTM3525XUpgradeable.getApproved(tokenId_) == operator_
        );
    }

    function _spendAllowance(address operator_, uint256 tokenId_, uint256 value_) internal virtual {
        uint256 currentAllowance = CTM3525XUpgradeable.allowance(tokenId_, operator_);
        if (!_isApprovedOrOwner(operator_, tokenId_) && currentAllowance != type(uint256).max) {
            require(currentAllowance >= value_, "CTM3525X: insufficient allowance");
    ...
    function _requireMinted(uint256 tokenId_) internal view virtual {
        require(_exists(tokenId_), "CTM3525X: invalid token ID");
    }
    ...
    function _mint(address to_, uint256 tokenId_, uint256 slot_, uint256 value_) internal virtual {
        require(to_ != address(0), "CTM3525X: mint to the zero address");
        require(tokenId_ != 0, "CTM3525X: cannot mint zero tokenId");
        require(!_exists(tokenId_), "CTM3525X: token already minted");
    ...
    function _burnValue(uint256 tokenId_, uint256 burnValue_) internal virtual {
        ...
        require(value >= burnValue_, "CTM3525X: burn value exceeds balance");
    ...
    function _approve(address to_, uint256 tokenId_) internal virtual {
        ...
        emit Approval(CTM3525XUpgradeable.ownerOf(tokenId_), to_, tokenId_);
    }
    ...
    function _approveValue(
        ...
        require(to_ != address(0), "CTM3525X: approve value to the zero address");
    ...
    function _transferValue(
        ...
        require(_exists(fromTokenId_), "CTM3525X: transfer from invalid token ID");
        require(_exists(toTokenId_), "CTM3525X: transfer to invalid token ID");
        ...
        require(fromTokenData.balance >= value_, "CTM3525X: insufficient balance for transfer");
        require(fromTokenData.slot == toTokenData.slot, "CTM3525X: transfer to token with different slot");
        ...
        require(
            _checkOnCTM3525XReceived(fromTokenId_, toTokenId_, value_, ""),
            "CTM3525X: transfer rejected by CTM3525XReceiver"
        );
    }
    ...
    function _transferTokenId(
        ...
        require(CTM3525XUpgradeable.ownerOf(tokenId_) == from_, "CTM3525X: transfer from invalid owner");
        require(to_ != address(0), "CTM3525X: transfer to the zero address");

        uint256 slot = CTM3525XUpgradeable.slotOf(tokenId_);
        uint256 value = CTM3525XUpgradeable.balanceOf(tokenId_);
    ...
    function _safeTransferTokenId(
        ...
        require(
            ...
            "CTM3525X: transfer to non ERC721Receiver"
        );
    }

    function _checkOnCTM3525XReceived( 
        uint256 fromTokenId_, 
        uint256 toTokenId_, 
        uint256 value_, 
        bytes memory data_
    ) internal virtual returns (bool) {
        address to = CTM3525XUpgradeable.ownerOf(toTokenId_);
        if (_isContract(to)) {
            try IERC165(to).supportsInterface(type(ICTM3525XReceiverUpgradeable).interfaceId) returns (bool retval) {
                if (retval) {
                    bytes4 receivedVal = ICTM3525XReceiverUpgradeable(to).onCTM3525XReceived(_msgSender(), fromTokenId_, toTokenId_, value_, data_);
                    return receivedVal == ICTM3525XReceiverUpgradeable.onCTM3525XReceived.selector;
                } else {
                    return true;
                }
    ...
    function _setMetadataDescriptor(address metadataDescriptor_) internal virtual {
        metadataDescriptor = ICTM3525XMetadataDescriptorUpgradeable(metadataDescriptor_);

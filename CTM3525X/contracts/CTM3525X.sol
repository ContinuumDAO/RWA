...
import "./IERC721.sol";
import "./ICTM3525X.sol";
import "./IERC721Receiver.sol";
import "./ICTM3525XReceiver.sol";
import "./extensions/IERC721Enumerable.sol";
import "./extensions/IERC721Metadata.sol";
import "./extensions/ICTM3525XMetadata.sol";
import "./periphery/interface/ICTM3525XMetadataDescriptor.sol";

contract CTM3525X is Context, ICTM3525XMetadata, IERC721Enumerable {
    ...
    ICTM3525XMetadataDescriptor public metadataDescriptor;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            ...
            interfaceId == type(ICTM3525X).interfaceId ||
            interfaceId == type(IERC721).interfaceId ||
            interfaceId == type(ICTM3525XMetadata).interfaceId ||
    ...
    function transferFrom(
        ...
        _mint(to_, newTokenId, CTM3525X.slotOf(fromTokenId_), 0);
    ...
    function approve(address to_, uint256 tokenId_) public payable virtual override {
        ...
        require(
            _msgSender() == owner || CTM3525X.isApprovedForAll(owner, _msgSender()),
    ...
    function tokenByIndex(uint256 index_) public view virtual override returns (uint256) {
        require(index_ < CTM3525X.totalSupply(), "CTM3525X: global index out of bounds");
    ...
    function tokenOfOwnerByIndex(address owner_, uint256 index_) public view virtual override returns (uint256) {
        require(index_ < CTM3525X.balanceOf(owner_), "CTM3525X: owner index out of bounds");
    ...
    function _isApprovedOrOwner(address operator_, uint256 tokenId_) internal view virtual returns (bool) {
        ...
        return (
            ...
            CTM3525X.getApproved(tokenId_) == operator_
        );
    }

    function _spendAllowance(address operator_, uint256 tokenId_, uint256 value_) internal virtual {
        uint256 currentAllowance = CTM3525X.allowance(tokenId_, operator_);
    ...
    function _approve(address to_, uint256 tokenId_) internal virtual {
        ...
        emit Approval(CTM3525X.ownerOf(tokenId_), to_, tokenId_);
    }
    ...
    function _transferTokenId(
        ...
        require(CTM3525X.ownerOf(tokenId_) == from_, "CTM3525X: transfer from invalid owner");
        require(to_ != address(0), "CTM3525X: transfer to the zero address");

        uint256 slot = CTM3525X.slotOf(tokenId_);
        uint256 value = CTM3525X.balanceOf(tokenId_);
    ...
    function _checkOnCTM3525XReceived( 
        ...
        address to = CTM3525X.ownerOf(toTokenId_);
        if (_isContract(to)) {
            try IERC165(to).supportsInterface(type(ICTM3525XReceiver).interfaceId) returns (bool retval) {
                if (retval) {
                    bytes4 receivedVal = ICTM3525XReceiver(to).onCTM3525XReceived(_msgSender(), fromTokenId_, toTokenId_, value_, data_);
                    return receivedVal == ICTM3525XReceiver.onCTM3525XReceived.selector;
                } else {
                    return true;
                }
    ...
    function _setMetadataDescriptor(address metadataDescriptor_) internal virtual {
        metadataDescriptor = ICTM3525XMetadataDescriptor(metadataDescriptor_);

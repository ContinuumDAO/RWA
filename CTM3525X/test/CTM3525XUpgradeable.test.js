...
async function deployCTM3525X(name, symbol, decimals) {
  const CTM3525XFactory = await ethers.getContractFactory('CTM3525XBaseMockUpgradeableWithInit');
  const CTM3525X = await CTM3525XFactory.deploy(name, symbol, decimals);
  await CTM3525X.deployed();
  return CTM3525X;
}

describe('CTM3525XUpgradeable', () => {
  ...
  shouldBehaveLikeCTM3525X('CTM3525XUpgradeable');
  shouldBehaveLikeCTM3525XMetadata('CTM3525XMetadataUpgradeable');

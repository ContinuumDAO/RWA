...
contract TestERC20Approval is Helpers {
    ...
    function setUp() public override {
        ...
        rwa1X.mintNewTokenValueLocal(user1, 0, testSlot, 1000, ID, feeTokenStr);
        rwa1X.mintNewTokenValueLocal(user1, 0, testSlot, 2000, ID, feeTokenStr);
        rwa1X.mintNewTokenValueLocal(user1, 0, testSlot, 3000, ID, feeTokenStr);
    ...
    function test_approveErc20_reverts_whenERC20NonExistent() public {
        ...
        vm.startPrank(tokenAdmin);
        string memory feeTokenStr = _toLower((address(usdc).toHexString()));
        rwa1X.mintNewTokenValueLocal(user1, 0, 3, 1000, ID, feeTokenStr);
        vm.stopPrank();
    ...
    function test_multipleUsersApprovalIsolation() public {
        ...
        vm.startPrank(tokenAdmin);
        string memory feeTokenStr = _toLower((address(usdc).toHexString()));
        rwa1X.mintNewTokenValueLocal(user2, 0, testSlot, 5000, ID, feeTokenStr);
        vm.stopPrank();

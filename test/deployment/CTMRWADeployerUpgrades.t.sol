...
contract TestCTMRWADeployerUpgrades is Helpers {
    ...
    function test_upgrade_proxy_successfully() public {
        ...
        (bool success,) = address(deployer).call(
            abi.encodeWithSignature(
                ...
                address(mockImpl),
                abi.encodeCall(MockCTMRWADeployerV2.initializeV2, (42))
            )
        );
    ...
    function test_upgrade_proxy_with_invalid_implementation_reverts() public {
        ...
        (bool success,) = address(deployer).call(
            abi.encodeWithSignature(
                "upgradeToAndCall(address,bytes)", invalidImpl, abi.encodeCall(MockCTMRWADeployerV2.initializeV2, (42))
            )
        );
        assertTrue(success, "upgradeToAndCall failed");
        vm.stopPrank();
    ...
    function test_fail_upgrade_proxy_same_version() public {
        ...
        (bool success,) = address(deployer).call(
            abi.encodeWithSignature(
                "upgradeToAndCall(address,bytes)",
                address(impl1),
                abi.encodeCall(MockCTMRWADeployerV2.initializeV2, (1))
            )
        );
        assertTrue(success);
        vm.stopPrank();
    ...
    function test_upgrade_proxy_multiple_upgrades() public {
        ...
        (bool success,) = address(deployer).call(
            abi.encodeWithSignature(
                "upgradeToAndCall(address,bytes)",
                address(impl2),
                abi.encodeCall(MockCTMRWADeployerV2.initializeV2, (2))
            )
        );
        assertTrue(success, "upgradeToAndCall failed");
        vm.stopPrank();
        ...
        vm.startPrank(gov);
        (success,) = address(deployer).call(
            abi.encodeWithSignature(
                "upgradeToAndCall(address,bytes)",
                address(impl3),
                abi.encodeCall(MockCTMRWADeployerV2.initializeV2, (3))
            )
        );
        assertTrue(success, "upgradeToAndCall failed");
        vm.stopPrank();

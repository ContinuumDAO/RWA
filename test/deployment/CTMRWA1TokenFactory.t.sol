...
contract CTMRWA1TokenFactoryTest is Test, Helpers {
    ...
    function invariant_DeterministicAddress() public {
        ...
        bytes memory bytecode = abi.encodePacked(type(CTMRWA1).creationCode, abi.encode(admin, ctmRwaMap, tokenName, symbol, decimals, baseURI, ctmRwa1X));
        bytes32 hash = keccak256(
            abi.encodePacked(bytes1(0xff), address(factory), salt, keccak256(bytecode))
        );

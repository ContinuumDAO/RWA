...
contract XChainTests is Script {
  ...
  function deployLocal() public returns (uint256) {
    ...
    uint256 IdBack = rwa1X.deployAllCTMRWA1X(
      true, 0, 1, 1, "Closed Roses", "ROSE", 18, "GFLD", chainIdsStr, feeTokenStr
    );
    console.log(IdBack);
  ...
  function deployRemote(uint256 _ID) public {
    ...
    uint256 IdBack = rwa1X.deployAllCTMRWA1X(
      false, _ID, 1, 1, tokenName, symbol, decimals, baseURI, toChainIdsStr, feeTokenStr
    );
  ...
  function createSlots(uint256 _ID, string[] memory chainIdsStr) public {
    ...
    /*bool ok = */rwa1X.createNewSlot(_ID, 0, "Fractional painting", chainIdsStr, feeTokenStr);
  ...
  function transferValueWholeTokenToAddress(uint256 _ID) public {
    ...
    rwa1X.transferWholeTokenX(
      admin.toHexString(), admin.toHexString(), "97", tokenId, _ID, feeTokenStr
    );
  ...
  function fundDividends(uint256 _ID, uint256 _slot) public returns (uint256) {
    ...
    /*uint256 dividendTotal = */ICTMRWA1Dividend(divAddr).getTotalDividend();
  ...
  function checkDeployData() public {
    ...
    /*(
      ...
      callData, (string, uint256, uint256, uint256, string, string, uint8, string, string)
    );
  ...
  function stringToAddress(string memory str) internal pure returns (address) {
    ...
    for (uint256 i = 0; i < 20; i++) {
      addrBytes[i] =
        bytes1(hexCharToByte(strBytes[2 + i * 2]) * 16 + hexCharToByte(strBytes[3 + i * 2]));
    }
